<x-admin.wrapper>
    ...
    <div>
        <x-admin.breadcrumb href="{{route('task.index')}}" title="{{ __('创建任务') }}">{{ __('<< 回到任务列表') }}</x-admin.breadcrumb>
        <x-admin.form.errors />
    </div>
    <div class="w-full py-2 bg-white overflow-hidden">
        ...
        @csrf
            ...
            <div class="py-2">
                <x-admin.form.label for="name" class="{{$errors->has('name') ? 'text-red-400' : ''}}">{{ __('活动描述') }}</x-admin.form.label>
            ...
            <div class="flex justify-end mt-4">
                ...
                <x-admin.form.button>{{ __('创建') }}</x-admin.form.button>
            </div>
        </form>
    </div>
</x-admin.wrapper>
